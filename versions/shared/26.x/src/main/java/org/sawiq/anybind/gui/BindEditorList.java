...
final class BindEditorList extends ContainerObjectSelectionList<BindEditorList.FormEntry>
    ...
    abstract class FormEntry extends ContainerObjectSelectionList.Entry<FormEntry> {
        ...
        final void drawLabel(GuiGraphicsExtractor graphics, Component label, int x, int y) {
            graphics.text(font, label, x, y, UiColors.TEXT);
        }
    ...
    private final class TextInputRow extends FormEntry {
        ...
        public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            field.extractRenderState(graphics, mouseX, mouseY, tickDelta);
            sideButtons.forEach(button -> button.extractRenderState(graphics, mouseX, mouseY, tickDelta));
        }
    ...
    private final class KeyRow extends FormEntry {
        ...
        public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            modifierButtons.forEach(button -> button.extractRenderState(graphics, mouseX, mouseY, tickDelta));
            keyButton.extractRenderState(graphics, mouseX, mouseY, tickDelta);
            clearButton.extractRenderState(graphics, mouseX, mouseY, tickDelta);
        }
    ...
    private final class ShellRow extends FormEntry {
        ...
        public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            graphics.text(font, trimmed(Component.translatable(action.getShell().descriptionKey()), formWidth),
    ...
    private final class HelpRow extends FormEntry {
        ...
        public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            for (int index = 0; index < Math.min(3, lines.size()); index++) {
                graphics.text(font, lines.get(index), formX, y + 4 + index * 10, UiColors.MUTED);
            }
        }
    }
